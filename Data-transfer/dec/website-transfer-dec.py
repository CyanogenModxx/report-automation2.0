...
def Data_transfer(database_info):
    ...
    for i,row in database_info.iterrows():
        ...
        cursor = DB.cursor()
        #查询用户信息
        customer_sql = "select email,uid,reg_time from zcshop_members where reg_time between 1575129600 and 1575216000 and email not like '%%taoten%%'AND email not like '%%@qq%%' AND email not like '%%adimin%%' AND email not like '%%@163.com%%' AND email not like '%%@126.com%%'"
