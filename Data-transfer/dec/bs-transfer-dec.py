...
def Data_transfer(database_info):
    ...
    for i,row in database_info.iterrows():
        ...
        cursor = DB.cursor()
        #查询用户信息
        customer_sql = "select email,id_customer,date_add from ps_customer where date_add like '2019-12-01%%' and email not like '%%taoten%%'AND email not like '%%@qq%%' AND email not like '%%adimin%%' AND email not like '%%@163.com%%' AND email not like '%%@126.com%%'  "
        ...
        #查询订单信息
        order_sql = "select id_order,o.id_customer,email,o.date_add,current_state,payment,total_paid,total_paid_real,device from ps_orders as o left join ps_customer as m on o.id_customer=m.id_customer  where o.date_add like '2019-12-01%%' and email not like '%%taoten%%'AND email not like '%%@qq%%' AND email not like '%%adimin%%' AND email not like '%%@163.com%%' AND email not like '%%@126.com%%' "
